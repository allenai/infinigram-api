#!/usr/bin/env python3
"""
Generate a CSV manifest for dolma3 `.npy` shards by invoking the AWS CLI.

Usage:
    python scripts/generate_dolma3_csv.py [--output PATH] [--prefix S3_URI]

Key options:
    --prefix          S3 prefix to scan (default: dolma3-dolmino-official path)
...
Example:
    python scripts/generate_dolma3_csv.py --output indexing/dolma3/dolma3.csv
"""
...
DOLMA3_PREFIX = "s3://ai2-llm/preprocessed/dolma3-dolmino-official/100B/allenai/dolma3-tokenizer/"
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate a CSV manifest for dolma3 `.npy` shards using `aws s3 ls` output."
        )
    )
    parser.add_argument(
        "--prefix",
        default=DOLMA3_PREFIX,
        help=f"S3 prefix to scan (default: {DOLMA3_PREFIX})",
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def parse_ls_output(prefix: str, stdout: str) -> Iterable[ObjectInfo]:
    ...
    for raw_line in stdout.splitlines():
        ...
        rows.append(ObjectInfo(key=full_key, size=size, subset="dolma3", category=category))
