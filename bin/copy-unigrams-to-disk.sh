...
echo "Creating unigram writer job for $INDEX_NAME"
# we use envsubst to replace variables in the kubectl yaml template
INDEX_NAME=$INDEX_NAME \
 INDEX_SIZE=$INDEX_SIZE \
 INDEX_BUCKET_NAME=$INDEX_BUCKET_NAME \
 DISK_NAME=$DISK_NAME \
 envsubst '$INDEX_NAME:$INDEX_SIZE:$INDEX_BUCKET_NAME:$DISK_NAME' < ./bin/infini-gram-writer/unigram-writer-job.yaml.template | kubectl apply --namespace=infinigram-api -f -
