...
def prepare_manyfiles_map(args, filenum, path):
    ...
    if path.endswith('.csv.gz'):
        ...
        for row in rows:
            ...
            assert tokens[end-1] == 100257  or tokens[end-1] == 100277, (npy_path, tokens[end-10:end])
...
def prepare_manyfiles(args):
    ...
    if not args.pretokenized:
        ...
        data_paths = list(sorted(glob.glob(f'{args.data_dir}/**/*.csv.gz', recursive=True)))
...
def prepare(args):
    ...
    elif args.prepare_mode == 'manyfiles':
        prepare_manyfiles(args)
    else:
        raise ValueError(f'Unknown prepare mode: {args.prepare_mode}')
